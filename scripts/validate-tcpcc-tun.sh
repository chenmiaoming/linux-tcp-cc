...
if [[ ! -x "$KERNEL" ]]; then
  echo "M5.2 requires an already-linked executable tcpcc vmlinux: $KERNEL" >&2
  exit 1
fi
if [[ ! -c /dev/net/tun ]]; then
  echo "M5.2 requires host /dev/net/tun" >&2
  exit 1
fi
for command in ip ping sudo; do
  if ! command -v "$command" >/dev/null 2>&1; then
    echo "M5.2 requires host command: $command" >&2
    exit 1
  fi
done
if ! sudo -n true; then
  echo "M5.2 CI adapter requires passwordless sudo for host TUN configuration" >&2
  exit 1
fi
...
rm -f "$BOOT_LOG" "$CONTROL_RESPONSES" "$PING_LOG"
...
printf 'M5.2 real TUN adapter and host IPv4 data path succeeded\n'
