...
#define TCPCC_HOST_MAP_PRIVATE            0x2
#define TCPCC_HOST_MAP_ANONYMOUS          0x20
#define TCPCC_HOST_MAP_FIXED_NOREPLACE    0x100000
...
long __init tcpcc_host_map_memory(unsigned long start, unsigned long end)
{
	...
	if (end <= start || ((start | end) & (PAGE_SIZE - 1)))
		return -EINVAL;

	ret = tcpcc_host_syscall6(TCPCC_HOST_NR_MMAP, start, end - start,
				  TCPCC_HOST_PROT_READ | TCPCC_HOST_PROT_WRITE,
				  TCPCC_HOST_MAP_PRIVATE |
				  TCPCC_HOST_MAP_ANONYMOUS |
				  TCPCC_HOST_MAP_FIXED_NOREPLACE,
				  -1L, 0);
	if (ret < 0)
		return ret;

	/*
	 * MAP_FIXED_NOREPLACE must either map the exact range or fail. Refuse a
	 * host that silently treats the flag as a hint rather than risking a
	 * non-identity physical mapping.
	 */
	if ((unsigned long)ret != start)
