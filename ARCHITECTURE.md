...
Running tcpcc as root inside the target VPS/container is an acceptable product
model. OpenVZ-style users normally administer their own guest as root; the
important compatibility question is whether that root actually has the needed
network authority (`CAP_NET_ADMIN`), TUN access, firewall support, and
forwarding. Privilege separation is therefore not a current product goal unless
a concrete deployment or security requirement later justifies it.
